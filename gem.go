...
func test() {
	/*
		...
		decryptedData := decryptAESFile(d2, key)
			...
			// Create encrypted file
			err = ioutil.WriteFile("/home/chaseweaver34/Code/gfe/img1.png", decryptedData, 0644)
			if err != nil {
				panic(err)
			}

			// Create encrypted file
			err = ioutil.WriteFile("/home/chaseweaver34/Code/gfe/output2.jpg", decryptedData, 0644)
			if err != nil {
				panic(err)
			}
	*/
